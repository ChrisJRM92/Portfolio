...
const CardProyect = ({data}) => {
  console.log(data)
  return (
    <div className="card_proyect" style={{ border: `1px solid ${data?.border_color}` }}>
      ...
      <div className="front_content">
        ...
        <div className='butom_proyect'>
          <a href={data?.github} target="_blank" style={{backgroundColor: `${data?.btn_color}`, color: `#e9e9f1`, display: `${data?.display}`}} className='btn_proyects'><p className="btn_github"><FaGithub /></p>&nbsp;&nbsp;{!data?.state_github ? 'Github' : data.state_github}</a>
          ...
          <a style={{backgroundColor: `${data?.backColor}`, color: `${data?.color_component}`, display: `${data?.display_icon}`, fontSize: `${data?.icon_size}`}} className="btn_proyects"><p className="star">{data?.component}</p></a>
        </div>
      </div>
    </div>
  )
